...
1- Prepare: An SQL statement template is created and sent to the database. 
   Certain values left unspecified are called parameters (labeled "?"). 
...
2- Execute: At a later time, the application binds the values to the 
...
1-Prepared statements reduce parsing time as the preparation 
  on the query is done only once (although the statement is 
  executed multiple times)

2-Bound parameters minimize bandwidth to the server as you need send 
  only the parameters each time, and not the whole query

3-Prepared statements are very useful against SQL injections, because 
...
In our SQL, we insert a question mark (?) where we want to substitute 
in an integer, string, double or blob value.
...
what the parameters are. The "sss" argument lists the types of data that 
the parameters are. The 's' character tells mysql that the parameter is a 
string.
...
By telling mysql what type of data to expect, we minimize the risk of 
SQL injections.
