...
Unlike a cookie, the information is not stored 
...
do some changes, and then you close it. This is 
much like a Session. The computer knows who you 
are. It knows when you start the application and 
when you end. 
...
user information to be used across multiple pages 
...
In this page, we start a new PHP session and set some 
session variables:
...
------->
  ...
  <!-- Note: The session_start() function must be the 
       ----- very first thing in your document. Before any HTML tags.
...
From this page, we will access the session information 
...
Also notice that all session variable values are stored in 
...
------->
  ...
  for a user session is to run the following code:
  ...
  Most sessions set a user-key on the user's computer that 
  looks something like this: 765487cf34ert8dede5a562e4f3a7e12. 
  
  Then, when a session is opened on another page, it scans 
  the computer for a user-key. If there is a match, it accesses 
  that session, if not, it starts a new session.
