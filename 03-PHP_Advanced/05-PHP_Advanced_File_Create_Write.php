...
<body>
  ...
  <!-- In this chapter we will teach you how to create and write 
...
Maybe a little confusing, but in PHP, a file is created using 
...
The file will be created in the same directory where the PHP 
code resides:
...
to write to and the second parameter is the string to be written.
...
Each time we wrote to the file we sent the string $txt 
that first contained "John Doe" and second contained "Jane Doe". 

After we finished writing, we closed the file using the 
'fclose()' function.
...
happens when we open an existing file for writing. 
All the existing data will be ERASED and we start with an 
empty file.

In the example below we open our existing file "newfile.txt", 
...
while the "w" mode overrides (and erases) the old 
content of the file.

In the example below we open our existing file "newfile.txt", 
and append some text to it:
...
For a complete reference of filesystem functions, go to our 
complete PHP Filesystem Reference. -->
</body>
